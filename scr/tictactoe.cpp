...
void TTT::ClearMap()
{																  // 틱텍토의 보드판을 초기화하는 함수
	map = vector<vector<char>>(NUM_CELL, vector<char>(NUM_CELL)); // 벡터의 1D, 2D 크기를 NUM_CELL로 설정하면서 초기화
...
{ // 컴퓨터의 선택을 결정하는 함수
	...
	{ // 유효한 숫자를 결정하기위한 루프
		...
		{									 // 유효한 숫자를 선택하면
			map[comx][comy] = current_player; // 틱텍토 보드에 컴퓨터의 돌을 둔다.
...
int TTT::PlayTTT()
{
	...
	num_user = 2;		 // 게임에 참가한 유저의 수
	current_player = ' '; // 현재 플레이어의 돌을 나타내는 변수
	is_win = false;		 // 우승자를 확인하는 변수
	x, y = 0;			 // 유효성 검사를 통과한 사용자의 입력(int)
