...
void MCP::SetInstruction()
{
    ...
    cout << "컴퓨터와 묵찌빠를 해 승리하면 점수를 얻습니다." << endl;
...
{ // 사용자로부터 입력을 받는 합수
    while (true)
    {
        ...
        // 입력한 유효한 숫자인지 확인
        // UserInput은 String이므로 Int로 캐스팅 해야한다., 위에서 유효성 검사 완료
        if (!IsValidNumber(stoi(user_input)))
        {
            cout << "잘못된 입력입니다. 1, 2, 3 중 하나를 입력하세요." << endl;
            continue;
        }
        return stoi(user_input); // 유효검사를 통과한 선택을 반납(int)
    }
}
...
int MCP::PlayMCP()
{
    ...
    while (true)
    {
        user_choice = GetUserInput();                          // 유효성검사를 통과한 사용자의 선택
